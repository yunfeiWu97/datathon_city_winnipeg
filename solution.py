...
def create_traffic_prediction_dashboard():
    ...
    # Create time slots for each day (hour intervals) - Reduced number of time slots for faster processing
    hours = list(range(6, 22, 1))  # 6 AM to 9 PM
    time_slots = [f"{hour:02d}:00" for hour in hours]
    
    # Generate fake traffic prediction data - more efficient generation
    traffic_data = []
    
    # Cache factors to avoid recalculating them
    day_factors = [0.8 + (date_idx * 0.1) for date_idx in range(len(prediction_dates))]
    road_factors = {road: 0.5 + random.random() for road in major_roads}
    
    # Pre-calculate time factors
    time_factors = {}
    for hour in hours:
        if 7 <= hour <= 9:  # Morning rush
            time_factors[hour] = 0.8 + (random.random() * 0.4)
        elif 16 <= hour <= 18:  # Evening rush
            time_factors[hour] = 0.9 + (random.random() * 0.3)
        elif 11 <= hour <= 13:  # Lunch hour
            time_factors[hour] = 0.6 + (random.random() * 0.3)
        else:  # Normal hours
            time_factors[hour] = 0.3 + (random.random() * 0.3)
    
    # Generate predictions using vectorized operations where possible
    for date_idx, pred_date in enumerate(prediction_dates):
        day_factor = day_factors[date_idx]
        weekday_factor = 0.7 if pred_date.weekday() >= 5 else 1.0  # Weekend or weekday
        
        for road in major_roads:
            road_factor = road_factors[road]
            
            for hour in hours:
                time_factor = time_factors[hour]
    ...
    # Create a figure with multiple components - reduced DPI for faster rendering
    ...
    # Improved pivot table creation
    for day_idx, day in enumerate(sorted(df['date'].unique())):
        day_data = df[df['date'] == day]
        
        # Use pivot_table instead of manual pivot
        ...
        # Use simplified heatmap for faster rendering
    ...
    # 2. Line plots for selected roads over time - optimize by pre-filtering data
    top_roads = df.groupby('road')['traffic_score'].mean().nlargest(5).index.tolist()
    ...
    for day_idx, day in enumerate(sorted(df['date'].unique())):
        ...
        # Plot data more efficiently by grouping road data
    ...
    # 3. Peak traffic predictions in a table - optimize data processing
    ax = plt.subplot(gs[:, 2])
    ax.axis('off')
    
    # Pre-filter the DataFrame for more efficient peak finding
    peak_times_table = []
    for day in sorted(df['date'].unique()):
        day_data = df[df['date'] == day]
        
        # Find morning peak (7-9 AM)
        morning_data = day_data[(day_data['hour'] >= 7) & (day_data['hour'] <= 9)]
        morning_peak = morning_data.loc[morning_data['traffic_score'].idxmax()]
        
        # Find evening peak (4-6 PM)
        evening_data = day_data[(day_data['hour'] >= 16) & (day_data['hour'] <= 18)]
        evening_peak = evening_data.loc[evening_data['traffic_score'].idxmax()]
    ...
    # Save the figure - reduced dpi for faster saving
    plt.savefig('winnipeg_traffic_prediction.png', dpi=200, bbox_inches='tight', facecolor='#222222')
    plt.close()
...
def create_traffic_map(df):
    ...
    # Calculate average congestion by road - simplified to improve performance
...
def create_traffic_map_only():
    """
    Creates only the traffic map without generating the large visualization image
    """
    # Set random seed for reproducibility
    np.random.seed(42)
...
if __name__ == "__main__":
    ...
    print("Running DeepTraffic DeepSeek V3 predictive model...")
